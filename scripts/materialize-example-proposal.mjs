...
export function extractProposalIssue(body) {
  ...
  const setupField = issueField(body, 'OpticalSetup share link', 'Contribution acknowledgement', { last: true });
  ...
  return { name, description, shareURL };
}
...
export function materializeProposal({ issueNumber, issueBody, userLogin, createdAt }) {
  ...
  return {
    proposal,
    proposalFile: `example-proposals/issue-${number}.json`,
    branchName: `example-proposal/issue-${number}`,
    commitSubject: `Propose example: ${deliveryName}`,
    prTitle: `Propose example: ${deliveryName}`,
    prBody: [
      `Community example proposal generated from #${number}.`,
      '',
      ...
      '',
      'This draft stores a reviewable proposal only. A maintainer must still assess the optical explanation and promote an accepted setup into the curated Examples menu.',
    ].join('\n'),
  };
}
...
function runFromEnvironment() {
  ...
  mkdirSync('example-proposals', { recursive: true });
