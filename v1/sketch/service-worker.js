// Cache names include the service-worker scope. Frozen /v1/sketch/ and future
// /v2/sketch/ installs therefore coexist instead of deleting each other.
const CACHE_SCOPE = new URL('./', self.location.href).pathname
  .replace(/^\/+|\/+$/g, '')
  .replace(/[^a-z0-9.-]+/gi, '-');
const CACHE_PREFIX = `opticalsetup-pwa-${CACHE_SCOPE || 'root'}-`;
